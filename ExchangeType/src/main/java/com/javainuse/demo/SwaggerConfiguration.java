...
public class SwaggerConfiguration {
	...
	public Docket productAPi() {
		return new Docket(DocumentationType.SWAGGER_2)
				...
				.select()
				.apis(RequestHandlerSelectors.basePackage("com.javainuse.demo.controller"))
				.paths(Predicates.not(PathSelectors.regex("/error.*"))).build()
				.groupName("ExchangeTypes")
				.apiInfo(metaInfo());
	}
	...
	private ApiInfo metaInfo() {
		...
		return new ApiInfoBuilder().title("Rest API Documentation").description("ExchangeTypes").version("1.0")
